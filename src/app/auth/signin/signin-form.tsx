...
export function AuthSignInForm({
  ...
  useEffect(() => {
    ...
    try {
      if (callbackUrl.startsWith("http")) {
        ...
        router.replace(`${u.pathname}${u.search}` || "/home");
      } else {
        router.replace(callbackUrl.startsWith("/") ? callbackUrl : "/home");
      }
    } catch {
      router.replace("/home");
    }
  ...
  const oauthHint =
    !oauth.google || !oauth.github
      ? "Add AUTH_GOOGLE_ID / AUTH_GOOGLE_SECRET and AUTH_GITHUB_ID / AUTH_GITHUB_SECRET (e.g. .env.local or Vercel → Environment Variables), then redeploy or restart dev."
      : null;
  ...
  const onEmailSignIn = async () => {
    ...
    try {
      ...
      try {
        if (callbackUrl.startsWith("http")) {
          const u = new URL(callbackUrl);
          router.push(`${u.pathname}${u.search}` || "/home");
        } else {
    ...
    <div className="mt-6 space-y-4">
      ...
      <div className="flex flex-col gap-2">
        <button
          ...
          onClick={() =>
            void signIn("google", { callbackUrl: absoluteAuthCallbackUrl(callbackUrl) })
          }
        ...
        <button
          ...
          onClick={() =>
            void signIn("github", { callbackUrl: absoluteAuthCallbackUrl(callbackUrl) })
          }
          className={oauthBtnClass}
        >
          ...
          <button
            ...
            onClick={() =>
              void signIn("microsoft-entra-id", {
                callbackUrl: absoluteAuthCallbackUrl(callbackUrl),
              })
            }
      ...
      <div className="space-y-3">
        ...
        {!supabaseReady && (
          <p className="text-xs text-[var(--fg-muted)]">
            Set <code className="text-[11px]">NEXT_PUBLIC_SUPABASE_*</code> for email login.
          </p>
        )}
      </div>
